...
def rank_grabber(beatmaps, rank):
    scores = []
    i = 0
...
def cunny_grabber(beatmaps, target):
    scores = []
    i = 0
    length = sum(len(beatmap.difficulties) for beatmap in beatmaps)
...
def main():
    ...
    if command == "1":
        ...
        target = input()
        cunny_grabber(map_scrape(), target)

    elif command == "3":
        fc_grabber(map_scrape())
    else:
